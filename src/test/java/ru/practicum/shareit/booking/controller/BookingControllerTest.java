...
import ru.practicum.shareit.common.storage.BookingController;
import ru.practicum.shareit.common.storage.BookingService;
import ru.practicum.shareit.item.comment.model.Item;
import ru.practicum.shareit.item.dto.ItemDto;
import ru.practicum.shareit.item.storage.Header;
import ru.practicum.shareit.item.storage.Util;
...
class BookingControllerTest {
	...
	@BeforeEach
	void setUp() {
		item = Item.builder().id(0L).name("item name").description("item description").available(true).ownerId(0L)
				.requestId(0L).build();

		user = User.builder().id(0L).name("username").email("username@email.com").build();

		booking = Booking.builder().id(0L).start(Util.now().plusMinutes(10)).end(Util.now().plusDays(1)).item(item)
	...
	void createBooking_whenInvoked_thenStatusIsOkAndBookingDtoReturned() {
		Long userId = 0L;
		InputBookingDto inputBookingDto = InputBookingDto.builder().start(booking.getStart()).end(booking.getEnd())
				.itemId(item.getId()).build();

		ItemDto itemDto = ItemDto.builder().id(item.getId()).name(item.getName()).description(item.getDescription())
				.available(item.getAvailable()).ownerId(item.getOwnerId()).requestId(item.getRequestId()).build();

		UserDto userDto = UserDto.builder().id(user.getId()).name(user.getName()).email(user.getEmail()).build();

		BookingDto bookingDto = BookingDto.builder().id(booking.getId()).start(booking.getStart()).end(booking.getEnd())
				.item(itemDto).booker(userDto).status(booking.getStatus()).build();

		when(bookingService.addBooking(inputBookingDto, userId)).thenReturn(booking);

		String result = mvc
				.perform(post("/bookings").header(Header.USER_ID, userId)
						.content(mapper.writeValueAsString(inputBookingDto)).contentType(MediaType.APPLICATION_JSON)
	...
	void createBooking_whenStartInThePast_thenStatusIsBadRequestAndServiceMethodNeverCalled() {
		Long userId = 0L;
		InputBookingDto inputBookingDto = InputBookingDto.builder().start(booking.getStart().minusHours(10))
				.end(booking.getEnd()).itemId(item.getId()).build();

		when(bookingService.addBooking(inputBookingDto, userId)).thenReturn(booking);

		mvc.perform(post("/bookings").header(Header.USER_ID, userId).content(mapper.writeValueAsString(inputBookingDto))
	...
	void createBooking_whenEndInThePast_thenStatusIsBadRequestAndServiceMethodNeverCalled() {
		Long userId = 0L;
		InputBookingDto inputBookingDto = InputBookingDto.builder().start(booking.getStart())
				.end(booking.getEnd().minusDays(10)).itemId(item.getId()).build();
	...
	void changeBookingStatus_whenInvoked_thenStatusIsOkAndServiceMethodCalled() {
		...
		mvc.perform(patch("/bookings/{bookingId}", bookingId).header(Header.USER_ID, ownerId).param("approved",
				String.valueOf(approved))).andExpect(status().isOk());
	...
	void getBooking_whenInvoked_thenStatusIsOkAndServiceMethodCalled() {
		...
		mvc.perform(get("/bookings/{bookingId}", bookingId).header(Header.USER_ID, userId)).andExpect(status().isOk());
	...
	void getAllUserBookings_whenInvoked_thenStatusIsOkAndServiceMethodCalled() {
		...
		mvc.perform(get("/bookings").header(Header.USER_ID, userId).param("state", state)
				.param("from", String.valueOf(from)).param("size", String.valueOf(size))).andExpect(status().isOk());
	...
	void getAllOwnerBookings_whenInvoked_thenStatusIsOkAndServiceMethodCalled() {
		...
		mvc.perform(get("/bookings/owner").header(Header.USER_ID, userId).param("state", state)
				.param("from", String.valueOf(from)).param("size", String.valueOf(size))).andExpect(status().isOk());
