...
class ItemRequestControllerTest {
	...
	void setUp() {
		...
		itemRequest = ItemRequest.builder().id(0L).requester(user).description("request description").build();

		itemRequestDto = ItemRequestDto.builder().id(itemRequest.getId()).description(itemRequest.getDescription())
				.build();
	}
	...
	void postItemRequest_whenInvoked_thenStatusIsOkAndItemRequestDtoReturned() {
		...
		String result = mvc
				.perform(post("/requests").header(Header.USER_ID, userId)
						.content(mapper.writeValueAsString(itemRequestDto)).characterEncoding(StandardCharsets.UTF_8)
	...
	void postItemRequest_whenDescriptionIsBlank_thenStatusIsBadRequestAndServiceMethodNeverCalled() {
		...
		mvc.perform(post("/requests").header(Header.USER_ID, userId).content(mapper.writeValueAsString(itemRequestDto))
	...
	void getRequestsByUser_whenInvoked_thenStatusIsOkServiceMethodCalled() {
		...
		mvc.perform(get("/requests").header(Header.USER_ID, userId)).andExpect(status().isOk());
	...
	void getAllRequests_whenInvoked_thenStatusIsOkAndServiceMethodCalled() {
		...
		mvc.perform(get("/requests/all").header(Header.USER_ID, userId).param("from", String.valueOf(from))
	...
	void getRequest_whenInvoked_thenStatusIsOkAndServiceMethodCalled() {
		...
		mvc.perform(get("/requests/{requestId}", requestId).header(Header.USER_ID, userId)).andExpect(status().isOk());
