...
class ItemRequestServiceImplTest {
	...
	void getAllRequests_whenInvoked_thenReturnItemRequestDtoWithItemsCollection() {
		...
		ItemRequestDtoWithItems dto1 = ItemRequestDtoWithItems.builder().id(0L).items(List.of(itemDtoForRequest))
				.build();
		List<ItemRequestDtoWithItems> expectedDtos = List.of(dto1);
		when(userService.getUser(anyLong())).thenReturn(user);
		ItemRequest request1 = ItemRequest.builder().id(0L).build();
		when(requestRepository.findAllByRequesterIdNotOrderByCreatedDesc(userId, page)).thenReturn(List.of(request1));
	...
	void getRequestsByUserId_whenInvoked_thenReturnItemRequestDtoWithItemsCollection() {
		...
		when(userService.getUser(anyLong())).thenReturn(user);
		ItemRequest request1 = ItemRequest.builder().id(0L).build();
		when(requestRepository.findAllByRequesterIdOrderByCreatedDesc(userId)).thenReturn(List.of(request1));
		when(itemRepository.findAllByRequestIdIn(anySet())).thenReturn(List.of(item));
	...
	void getRequest_whenInvoked_thenReturnItemRequestDtoWithItems() {
		...
		ItemRequestDtoWithItems expectedDto = ItemRequestDtoWithItems.builder().items(List.of(itemDtoForRequest))
				.build();
		when(userService.getUser(anyLong())).thenReturn(user);
