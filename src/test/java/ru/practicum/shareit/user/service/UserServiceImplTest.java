...
import ru.practicum.shareit.user.storage.UserServiceImpl;
...
class UserServiceImplTest {
	...
	void updateUser_whenNameNotBlankAndEmailNotNull_ThenReturnUserWithUpdatedFields() {
		Long userId = 0L;
		User oldUser = User.builder().name("name").email("email@mail.com").build();

		User newUser = User.builder().name("new name").email("newemail@mail.com").build();
	...
	void updateUser_whenNameIsBlankAndEmailIsNull_ThenReturnUserWithOldFields() {
		...
		User newUser = User.builder().name(" ").email(null).build();
		when(userRepository.findById(userId)).thenReturn(Optional.of(oldUser));
