...
public class ItemService {
	...
	@Autowired
	public ItemService(ItemRepository itemRepository, UserService userService, @Lazy BookingService bookingService,
	...
	public ItemAllDto getItem(Long id, Long userId) {
		...
		return ItemMapper.toItemAllDto(
	...
	public List<ItemAllDto> getItems(Long userId) {
		...
		Map<Long, List<BookingDto>> bookingsMap = bookingService.getBookingsByOwner(userId, Optional.empty()).stream()
				.collect(Collectors.groupingBy((BookingDto booking) -> booking.getItem().getId()));
		return itemRepository.findAllByOwnerId(userId).stream()
				.map(item -> ItemMapper.toItemAllDto(item, findLast(bookingsMap.get(item.getId())),
						findNext(bookingsMap.get(item.getId())), findComments(item.getId())))
				.toList();
	}

	private List<CommentDto> findComments(Long itemId) {
		return commentRepository.findAllByItem_IdOrderByCreated(itemId).stream().map(CommentMapper::toCommentDto)
				.toList();
	}
	...
	private BookingDto findLast(List<BookingDto> booking) {
		if (booking == null) {
			...
			return booking.stream().filter(bk -> bk.getEnd().isBefore(LocalDateTime.now()))
	...
	private BookingDto findNext(List<BookingDto> booking) {
		if (booking == null) {
			...
			return booking.stream().filter(bk -> bk.getStart().isAfter(LocalDateTime.now()))
