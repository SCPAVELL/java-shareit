...
import ru.practicum.shareit.item.user.mapper.UserMapper;
...
public class BookingService {
	...
	public List<BookingDto> getBookings(Long userId, Optional<BookingState> bState) {
		...
		switch (state) {
		case ALL:
			return repository.findByBooker_IdOrderByStartDesc(userId).stream().map(BookingMapper::toBookingDto)
					.toList();
		case CURRENT:
			return repository.findByBooker_IdAndStatusOrderByStartDesc(userId, BookingStatus.APPROVED).stream()
	...
	public List<BookingDto> getBookingsByOwner(Long userId, Optional<BookingState> bState) {
		...
		switch (state) {
		case ALL:
			return repository.findBookingsByItemOwnerOrderByStartDesc(user).stream().map(BookingMapper::toBookingDto)
					.toList();
		case CURRENT:
			return repository.findBookingsByItemOwnerAndStatusOrderByStartDesc(user, BookingStatus.APPROVED).stream()
					.map(BookingMapper::toBookingDto).toList();
		case PAST:
			return repository.findBookingsByItemOwnerAndEndIsBeforeOrderByStartDesc(user, LocalDateTime.now()).stream()
					.map(BookingMapper::toBookingDto).toList();
		case FUTURE:
			return repository.findBookingsByItemOwnerAndStartIsAfterOrderByStartDesc(user, LocalDateTime.now()).stream()
					.map(BookingMapper::toBookingDto).toList();
		case WAITING:
			return repository.findBookingsByItemOwnerAndStatusOrderByStartDesc(user, BookingStatus.WAITING).stream()
					.map(BookingMapper::toBookingDto).toList();
		case REJECTED:
			return repository.findBookingsByItemOwnerAndStatusOrderByStartDesc(user, BookingStatus.REJECTED).stream()
					.map(BookingMapper::toBookingDto).toList();
		default:
			throw new BadRequestException("Booking status: " + state + " unsupported!");
		}
	...
	public List<BookingDto> getBookingsByUserAndItem(Long userId, Long itemId) {
		return repository.findByBooker_IdAndItem_IdAndStatusAndEndIsBefore(userId, itemId, BookingStatus.APPROVED,
