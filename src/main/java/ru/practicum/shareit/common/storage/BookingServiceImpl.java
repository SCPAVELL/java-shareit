...
public class BookingServiceImpl implements BookingService {
	...
	public Booking addBooking(InputBookingDto inputBookingDto, Long bookerId) {
		log.debug("Обработка запроса на бронирование вещи={} от пользователя={}", inputBookingDto.getItemId(),
				bookerId);
		User user = userService.getUser(bookerId);
		Item item = itemRepository.findById(inputBookingDto.getItemId())
				.orElseThrow(() -> new ResourceNotFoundException(
						String.format("Вещь с id=%d не найдена", inputBookingDto.getItemId())));
	...
	public Booking updateBooking(Long ownerId, Long bookingId, Boolean approved) {
		log.debug("Обработка запроса на изменение статуса бронирования id={} от пользователя={}", bookingId, ownerId);
		Booking booking = getBooking(bookingId);
		...
		Booking bookingForUpdate = booking.toBuilder()
				.status(approved ? BookingStatus.APPROVED : BookingStatus.REJECTED).build();
	...
	public Collection<Booking> getAllUserBookingsByState(Long userId, String state, Integer from, Integer size) {
		...
		case CURRENT:
			bookingsByState = bookingRepository
					.findByBookerIdAndStartLessThanEqualAndEndGreaterThanEqualOrderByStartDesc(userId, Util.now(),
							Util.now(), page);
			break;
		case FUTURE:
			bookingsByState = bookingRepository.findByBookerIdAndStartAfterOrderByStartDesc(userId, Util.now(), page);
			break;
		case PAST:
			bookingsByState = bookingRepository.findByBookerIdAndEndBeforeOrderByStartDesc(userId, Util.now(), page);
			break;
		case WAITING:
			bookingsByState = bookingRepository.findByBookerIdAndStatusOrderByStartDesc(userId, BookingStatus.WAITING,
					page);
			break;
		case REJECTED:
			bookingsByState = bookingRepository.findByBookerIdAndStatusOrderByStartDesc(userId, BookingStatus.REJECTED,
					page);
			break;
		default:
	...
	public Collection<Booking> getAllOwnerBookingsByState(Long ownerId, String state, Integer from, Integer size) {
		...
		case CURRENT:
			bookingsByState = bookingRepository
					.findByItemOwnerIdAndStartLessThanEqualAndEndGreaterThanEqualOrderByStartDesc(ownerId, Util.now(),
							Util.now(), page);
			break;
		case FUTURE:
