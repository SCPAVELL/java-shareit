...
class ItemControllerTest {
	...
	void postItem_whenInvoked_thenStatusIsOkAndItemDtoReturned() {
		ItemDto dto = ItemDto.builder().id(item.getId()).name(item.getName()).description(item.getDescription())
		...
		String result = mvc
				.perform(post("/items").content(mapper.writeValueAsString(dto))
						.characterEncoding(StandardCharsets.UTF_8).contentType(MediaType.APPLICATION_JSON)
						.accept(MediaType.APPLICATION_JSON).header(Header.USER_ID, ownerId))
	...
	void postItem_whenItemDtoHasBlankName_thenStatusIsBadRequestAndServiceMethodNeverCalled() {
		ItemDto dto = ItemDto.builder().id(item.getId()).name(" ").description(item.getDescription())
				.available(item.getAvailable()).ownerId(item.getOwnerId()).requestId(item.getRequestId()).build();
		Long ownerId = 0L;

		mvc.perform(post("/items").content(mapper.writeValueAsString(dto)).characterEncoding(StandardCharsets.UTF_8)
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)
				.header(Header.USER_ID, ownerId)).andExpect(status().isBadRequest());
	...
	void postItem_whenItemDtoHasBlankDescription_thenStatusIsBadRequestAndServiceMethodNeverCalled() {
		ItemDto dto = ItemDto.builder().id(item.getId()).name(item.getName()).description(" ")
				.available(item.getAvailable()).ownerId(item.getOwnerId()).requestId(item.getRequestId()).build();
		Long ownerId = 0L;
	...
	void postItem_whenItemDtoAvailableIsNull_thenStatusIsBadRequestAndServiceMethodNeverCalled() {
		ItemDto dto = ItemDto.builder().id(item.getId()).name(item.getName()).description(item.getDescription())
				.available(null).ownerId(item.getOwnerId()).requestId(item.getRequestId()).build();
		Long ownerId = 0L;
	...
	void getItem_whenInvoked_StatusIsOkAndServiceMethodCalled() {
		...
		mvc.perform(get("/items/{itemId}", itemId).header(Header.USER_ID, userId)).andExpect(status().isOk());
	...
	void getAllItems_whenInvoked_thenStatusIsOkAndServiceMethodCalled() {
		...
		mvc.perform(get("/items").header(Header.USER_ID, userId).param("from", String.valueOf(from)).param("size",
	...
	void getAllItems_whenFromIsNegative_thenStatusIsAndServiceMethodCalledWithDefaultValues() {
		...
		mvc.perform(get("/items").header(Header.USER_ID, userId).param("from", String.valueOf(from)).param("size",
				String.valueOf(size))).andExpect(status().isBadRequest());
	...
	void patchItem_whenInvoked_thenStatusIsOkAndServiceMethodCalled() {
		...
		String result = mvc
				.perform(patch("/items/{itemId}", itemId).content(mapper.writeValueAsString(dto))
						.characterEncoding(StandardCharsets.UTF_8).contentType(MediaType.APPLICATION_JSON)
						.accept(MediaType.APPLICATION_JSON).header(Header.USER_ID, ownerId))
				.andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
	...
	void deleteItem_whenInvoked_thenStatusIsOk() {
		...
		mvc.perform(delete("/items/{itemId}", itemId).header(Header.USER_ID, userId)).andExpect(status().isOk());
	...
	void searchItems_whenInvoked_thenStatusIsOkAndServiceMethodCalled() {
		...
		mvc.perform(get("/items/search").param("text", text).param("from", String.valueOf(from)).param("size",
				String.valueOf(size))).andExpect(status().isOk());
	...
	void postComment_whenInvoked_thenStatusIsOkAndCommentDtoReturned() {
		...
		String result = mvc
				.perform(post("/items/{itemId}/comment", itemId).content(mapper.writeValueAsString(commentDto))
						.characterEncoding(StandardCharsets.UTF_8).contentType(MediaType.APPLICATION_JSON)
						.accept(MediaType.APPLICATION_JSON).header(Header.USER_ID, userId))
				.andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
	...
	void postComment_whenCommentTextIsBlank_thenStatusIsBadRequestAndServiceMethodNeverCalled() {
		...
		mvc.perform(post("/items/{itemId}/comment", itemId).content(mapper.writeValueAsString(commentDto))
				.characterEncoding(StandardCharsets.UTF_8).contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON).header(Header.USER_ID, userId)).andExpect(status().isBadRequest())
