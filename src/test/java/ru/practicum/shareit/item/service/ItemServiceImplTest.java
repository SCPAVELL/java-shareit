...
import ru.practicum.shareit.item.dto.CommentRepository;
...
class ItemServiceImplTest {
	...
	void addItem_whenOwnerNotFound_thenNotFoundExceptionThrown() {
		...
		assertThrows(ResourceNotFoundException.class, () -> itemService.addItem(item, ownerId));
	...
	void getItem_whenItemNotFound_thenNotFoundExceptionThrown() {
		...
		assertThrows(ResourceNotFoundException.class, () -> itemService.getItem(itemId, userId));
	}
	...
	void updateItem_whenItemNotFound_thenNotFoundExceptionThrown() {
		...
		assertThrows(ResourceNotFoundException.class, () -> itemService.updateItem(item, itemId, userId));
	...
	void updateItem_whenOwnerIdDoesNotMatch_thenPermissionDeniedExceptionThrown() {
		...
		assertThrows(PermissionDeniedException.class, () -> itemService.updateItem(item, itemId, userId));
	...
	void updateItem_whenNameNotBlankAndDescriptionNotBlankAndAvailableNotNull_thenReturnItemWithUpdatedFields() {
		Long itemId = 0L;
		Long ownerId = 0L;
		Item oldItem = Item.builder().name("name").description("description").available(false).ownerId(ownerId).build();

		Item newItem = Item.builder().name("name1").description("description1").available(true).build();
	...
	void updateItem_whenNameIsBlankAndDescriptionIsBlankAndAvailableIsNull_thenReturnItemWithOldFields() {
		...
		Item newItem = Item.builder().name(" ").description(" ").available(null).build();
		when(itemRepository.findById(itemId)).thenReturn(Optional.of(oldItem));
	...
	void deleteItem_whenItemNotFound_thenNotFoundExceptionThrown() {
		...
		assertThrows(ResourceNotFoundException.class, () -> itemService.deleteItem(itemId, ownerId));
	...
	void addComment_whenUserIdEqualsOwnerId_thenValidationExceptionThrown() {
		...
		assertThrows(ResourceValidationException.class, () -> itemService.addComment(commentDto, itemId, userId));
