...
public class ItemServiceImpl implements ItemService {
	...
	public OwnerItemDto getItem(Long itemId, Long userId) {
		...
		return ItemMapper.toOwnerItemDto(item, bookingService.getLastBooking(itemId),
				bookingService.getNextBooking(itemId), comments);
	}
	...
	public Collection<OwnerItemDto> getAllItems(Long ownerId, Integer from, Integer size) {
		...
		return itemRepository.findByOwnerId(ownerId, page).stream()
				.map(i -> ItemMapper.toOwnerItemDto(i, bookingService.getLastBooking(i.getId()),
						bookingService.getNextBooking(i.getId()), Collections.emptyList()))
	...
	public Item updateItem(Item item, Long itemId, Long ownerId) {
		...
		Item itemForUpdate = itemInDb.toBuilder()
				...
				.available(item.getAvailable() == null ? itemInDb.getAvailable() : item.getAvailable()).build();
	...
	public CommentDto addComment(CommentDto commentDto, Long itemId, Long userId) {
		...
		if (item.getOwnerId().equals(user.getId()) || bookingService.getPastUserBookings(itemId, userId).isEmpty()) {
